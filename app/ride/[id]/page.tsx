...
export default function RideNavigationPage() {
  ...
  useEffect(() => {
    ...
    const fetchRoutes = async (startLat: number, startLng: number) => {
      try {
        const apiKey = process.env.NEXT_PUBLIC_TOMTOM_API_KEY;
        const res = await fetch(`https://api.tomtom.com/routing/1/calculateRoute/${startLat},${startLng}:${destLat},${destLng}/json?key=${apiKey}&maxAlternatives=2`);
        if (res.ok) {
          ...
          if (data.routes && data.routes.length > 0) {
            ...
            const routeGeoJsons = data.routes.map((r: any) => {
              return {
                ...
                geometry: {
                  type: 'LineString',
                  coordinates: r.legs[0].points.map((p: any) => [p.longitude, p.latitude])
                }
              };
            });
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          fetchRoutes(userLat, userLng);
        },
        (error) => {
          console.error("Error getting location", error);
          // Fallback if desktop GPS fails (Default to SF)
      ...
      watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Update the user's marker position in real-time
          setMapMarkers(prev => {
            return prev.map(m => {
              if (m.id === 'start') {
  ...
  }, [destLatStr, destLngStr, user, isNavigating]);
    ...
    <div className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      <FadeContent blur={true} duration={1000} delay={0.4} ease="ease-out" initialOpacity={0} className="absolute right-4 top-1/4 md:top-1/2 md:-translate-y-1/2 z-20 pointer-events-none flex flex-col items-end space-y-3">
        ...
        >
          ...
          <span className="text-xs font-bold text-white">1/{memberLimit}</span>
        </button>
        ...
        {/* Cancel Ride (Admin Only) - Moved to Members Panel */}
      </FadeContent>
          ...
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0a0a0a] shadow-2xl relative flex flex-col max-h-full">
            <div className="p-5 border-b border-white/5 flex items-center justify-between shrink-0">
              <h2 className="text-xl font-black text-white tracking-tight uppercase flex items-center">
                <Users className="w-5 h-5 mr-2 text-blue-400" /> Riders <span className="ml-2 text-emerald-400 font-bold">{1}/{memberLimit}</span>
              </h2>
        ...
        >
          ...
          >
            ...
            <div className="w-56 h-56 md:w-64 md:h-64 bg-black rounded-2xl flex items-center justify-center text-white p-4">
               {/* Using a giant QrCode icon as a placeholder. In a real app this would be a generated QR image */}
